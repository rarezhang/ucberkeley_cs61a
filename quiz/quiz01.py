...
def nearest_two(x):
    ...
    if x > 1:
        ...
        power = 0.5
    while abs(power_of_two * power - x) < abs(power_of_two - x):  #loop if there is still room for growth
        power_of_two *= power

    if abs(power_of_two * 2 - x ) == abs(power_of_two - x): # when there is a tie
